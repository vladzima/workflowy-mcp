...
class TestSearchAndFilter:
    ...
    async def test_search_with_special_characters(self) -> None:
        ...
        node1 = await create_node.fn(
            name="Code: function()",
            note="Implementation of foo()"
        )
        
        node2 = await create_node.fn(
            name="Email@example.com",
            note="Contact information"
        )
        
        node3 = await create_node.fn(
            name="Price: $99.99",
            note="Product pricing"
        )
    ...
    async def test_list_with_depth_limit(self) -> None:
        ...
        level1 = await create_node.fn(
            name="Level 1",
            parent_id=root_id
        )
        
        level2 = await create_node.fn(
            name="Level 2",
            parent_id=level1["node"]["id"]
        )
        
        level3 = await create_node.fn(
            name="Level 3",
            parent_id=level2["node"]["id"]
        )
